...
class InstallmentModel extends Model {
    ...
    public function getInstallmentDownPayments() {
        $installmentsQuery = $this->db->table("installments AS i")
            ->select("'lot' AS asset_type, i.lot_id AS asset_id, i.down_payment, i.down_payment_status, i.down_payment_date, i.down_payment_due_date")
            ->where("down_payment_status", "Pending")
            ->getCompiledSelect();

        $estateInstallmentsQuery = $this->db->table("estate_installments AS ei")
            ->select("'estate' AS asset_type, ei.estate_id AS asset_id, ei.down_payment, ei.down_payment_status, ei.down_payment_date, ei.down_payment_due_date")
            ->getCompiledSelect();

        $query = $this->db->query("$installmentsQuery UNION $estateInstallmentsQuery");
    ...
    public function setDownPayment($assetIdType, $tableName, $assetId, $assetIdKey) {
        ...
        if ($updated) {
            ...
            return ["success" => false, "message" => "Failed to updated payment."];
        }
    }

    // public function setMonthlyPayment($assetIdType, $installmentId, $paymentAmount, $receiptPath, $tableName, $assetId, $assetIdKey) {
    //     $data = [
